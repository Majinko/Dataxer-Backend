...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    public Invoice generateTaxDocument(Long proformaInvoiceId) {
        ...
        BigDecimal payed = this.getPayedValueFromRelatedType(proformaInvoiceId, DocumentType.TAX_DOCUMENT);
        BigDecimal payments = this.getPaymentsValue(proformaInvoiceId);
        ...
        for (Integer keyTax : sortedKeys) {
            if (payments.compareTo(BigDecimal.ZERO) > 0 && valuesForTaxes.get(keyTax).compareTo(payed) > 0) {
                ...
                taxDocumentPack.setTitle(this.generateTaxDocumentPackTitle(getNewestPaymentPayedDate(proformaInvoiceId), proformaInvoice.getVariableSymbol()));
